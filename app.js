...
(function(){
  ...
  const oldFn = (typeof window!=='undefined' && window.isVariantAllowed) ? window.isVariantAllowed : null;
  window.isVariantAllowed = function(type, strengthClass, variant){
    const v = String(variant||'').trim().toUpperCase();
    if (allowed(type).includes(v)) return true;
    return oldFn ? oldFn(type, strengthClass, v) : false;
  };
})();
